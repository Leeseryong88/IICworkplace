...
import { X, Sparkles, Image, RefreshCw, Layers } from 'lucide-react';
import { PosterContent, IconName } from '../types';
...
interface PosterEditorProps {
  ...
  onAiGenerate: (context: string) => Promise<void>;
        ...
        >
          ...
          <div className="flex justify-between items-center px-6 py-3 border-b-2 border-black bg-white text-black">
            <div className="flex items-center gap-2">
              <div className="w-6 h-6 bg-black flex items-center justify-center text-white font-black text-[10px]">S_G</div>
              <h2 className="text-lg font-[900] uppercase tracking-[-0.02em]">안전 수칙 에디터</h2>
            </div>
            <button onClick={onClose} className="p-1 hover:bg-black hover:text-white transition-all border-2 border-transparent hover:border-black">
              <X className="w-5 h-5" />
            </button>
          </div>
          ...
          <div className="flex-1 overflow-y-auto px-6 py-6 space-y-8">
            ...
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <div className="space-y-1">
                  <label className="text-[0.6rem] font-black uppercase tracking-[0.2em] opacity-60">포스터 메인 문구</label>
                  <textarea 
                    ...
                    <div className="flex justify-between items-center pb-2 border-b border-black/10">
                      ...
                      <div className="flex gap-2">
                        ...
                        <button 
                          ...
                          className={`w-10 h-10 border-2 border-black flex items-center justify-center transition-all ${action.imageUrl ? 'bg-black text-white' : 'bg-white text-black hover:bg-black hover:text-white'}`}
                          title="이미지 업로드"
                        >
                        ...
                        <IconSelector 
                          ...
                          onSelect={(name) => {
                            ...
                            newActions[idx].imageUrl = undefined; // Clear image if icon selected
                            onUpdate({ coreActions: newActions });
                          }}
                        />
                      </div>
            ...
            <div className="space-y-4">
              <div className="flex items-center gap-3">
                ...
                <button 
                  onClick={() => {
                    const nextId = content.detailedRules.length > 0 ? Math.max(...content.detailedRules.map(r => r.id)) + 1 : 1;
              ...
              <div className="grid grid-cols-3 gap-3">
                {content.detailedRules.map((rule, idx) => (
                  <div key={rule.id} className="flex gap-2 items-center bg-white p-2 border-2 border-black group hover:border-red-500 transition-all">
          ...
          <div className="px-6 py-4 border-t-2 border-black bg-white flex justify-end items-center">
            <button 
              onClick={onClose}
              className="px-8 py-2.5 bg-black text-white border-2 border-black font-black uppercase text-xs tracking-widest hover:bg-white hover:text-black transition-all shadow-[4px_4px_0px_rgba(0,0,0,1)]"
            >
              편집 완료 [완료]
            </button>
          </div>
...
const IconSelector = ({ current, onSelect }: { current: IconName, onSelect: (name: IconName) => void }) => {
  const [isOpen, setIsOpen] = useState(false);
  const iconList: IconName[] = [
    'HardHat', 'ShieldAlert', 'Wrench', 'Construction', 'Zap', 'Flame', 'Thermometer', 
    'Wind', 'Droplets', 'Eye', 'Stethoscope', 'Volume2', 'CigaretteOff', 'Hand', 
    'Siren', 'Info', 'AlertTriangle', 'Lock', 'PlugZap', 'User', 'Skull', 'Biohazard', 
    'Radiation', 'Factory', 'Truck', 'Scale', 'Anchor', 'Hammer', 'Boxes', 'Package', 
    'ClipboardList', 'CheckCircle2', 'XCircle', 'Ban', 'Activity', 'Ear', 'EarOff', 
    'LifeBuoy', 'Snowflake', 'Sun', 'Umbrella', 'UtilityPole', 'Waves', 'Trash2', 'Dna'
  ] as IconName[];
        ...
        <div className="absolute top-full right-0 mt-2 p-3 bg-white border-2 border-black shadow-[10px_10px_0px_rgba(0,0,0,1)] grid grid-cols-6 gap-2 z-[70] w-[300px] max-h-[300px] overflow-y-auto">
          {iconList.map((name) => (
            <button 
              ...
              className={`aspect-square p-1 border-2 flex items-center justify-center transition-all ${current === name ? 'bg-black text-white border-black' : 'text-black border-transparent hover:border-black'}`}
              title={name}
            >
...
const IconRenderer = ({ name, ...props }: { name: IconName } & any) => {
  const IconComponent = (Icons as any)[name] || Icons.AlertCircle;
